...
import sbft.consensus.TimestampValuePair;
...
public class LCManager {
    ...
    public boolean quorumHighest(int timestamp, byte[] value, HashSet<CollectData> collects) {
        ...
        for (CollectData c : collects) {
            ...
            //sbft.tom.util.Logger.println("\t\t[QUORUM HIGHEST] ts' < ts : " + (c.getQuorumWrites().getTimestamp() < timestamp));
            //sbft.tom.util.Logger.println("\t\t[QUORUM HIGHEST] ts' = ts && val' = val : " + (c.getQuorumWrites().getTimestamp() == timestamp && Arrays.equals(value, c.getQuorumWrites().getValue())));
...
//                sbft.tom.util.Logger.println("\t\t[CERTIFIED VALUE] " + pv.getTimestamp() + "  >= " + timestamp);
//                sbft.tom.util.Logger.println("\t\t[CERTIFIED VALUE] " + Arrays.toString(value) + "  == " + Arrays.toString(pv.getValue()));
...
//        byte[] hashedValue = md.digest(cDec.getDecision());
        ...
        return Thresig.verify(cDec.getDecision(), sigShareMessages.toArray(new SigShareMessage[0]), sigShareMessages.size(), tomLayer.controller.getCurrentViewN(), cDec.getN(), cDec.getE());
